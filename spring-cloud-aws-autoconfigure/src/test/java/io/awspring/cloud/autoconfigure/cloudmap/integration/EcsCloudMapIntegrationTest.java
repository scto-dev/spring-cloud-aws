...
import static io.awspring.cloud.cloudmap.CloudMapUtils.ECS;
import static io.awspring.cloud.cloudmap.CloudMapUtils.ECS_CONTAINER_METADATA_URI_V_4;
...
public class EcsCloudMapIntegrationTest {
	...
	private static final LocalStackContainer localStackContainer = new LocalStackContainer(
			...
			.withEnv(IntegrationTestUtil.LOCAL_STACK_API, IntegrationTestUtil.LOCAL_STACK_API_KEY).withReuse(true);
	...
	void registerEcsContainerWithCloudMapWithNoSpecification() {
		...
		try (ConfigurableApplicationContext context = application.run(properties.toArray(new String[0]))) {
			...
			Assertions.assertNotNull(
					cloudMapUtils.getNameSpaceId(serviceDiscoveryClient, CloudMapUtils.DEFAULT_NAMESPACE));
			...
			Assertions.assertEquals(1,
					IntegrationTestUtil.getCloudMapRegisteredInstances(localStackContainer, serviceId));
		}
	}
	...
	void discoverEcsCloudMapInstancesWithNoSpecification() {
		...
		List<String> properties = IntegrationTestUtil.getCloudMapSpringBootProperties(localStackContainer);
		properties.add("--spring.cloud.aws.cloudmap.discovery.discoveryList[0].nameSpace="
				+ IntegrationTestUtil.TEST_DEFAULT_NAMESPACE);
		properties.add("--spring.cloud.aws.cloudmap.discovery.discoveryList[0].service="
				+ IntegrationTestUtil.TEST_DEFAULT_SERVICE);
		properties.add("--spring.cloud.aws.cloudmap.deploymentPlatform=" + ECS);
