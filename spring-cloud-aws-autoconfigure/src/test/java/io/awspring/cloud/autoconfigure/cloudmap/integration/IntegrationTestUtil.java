...
public class IntegrationTestUtil {
	...
	public static void createSubnet(LocalStackContainer localStackContainer, String vpcId) {
		try {
			localStackContainer.execInContainer("awslocal", "ec2", "create-subnet", "--vpc-id", vpcId, "--cidr-block",
					CIDR_BLOCK, "--region", REGION);
		}
		catch (IOException | InterruptedException e) {
			throw new RuntimeException(e);
		}
	...
	public static void createCloudMapResources(LocalStackContainer localStackContainer, String namespace,
		...
		try {
			// Create namespace
			localStackContainer.execInContainer("awslocal", "servicediscovery", "create-private-dns-namespace",
					"--name", namespace, "--region", REGION, "--vpc", vpcId);

			// List namespaces and get the id
			final Container.ExecResult listNameSpaceOutput = localStackContainer.execInContainer("awslocal",
					"servicediscovery", "list-namespaces", "--region", REGION);
			final String namespaceId = listNameSpaceOutput.getStdout().split("Id")[1].split("\"")[2];

			// Create service
			final String serviceId = localStackContainer
					.execInContainer("awslocal", "servicediscovery", "create-service", "--name", service,
							"--namespace-id", namespaceId, "--region", REGION)
					.getStdout().split("Id")[1].split("\"")[2];
			localStackContainer.execInContainer("awslocal", "servicediscovery", "register-instance", "--service-id",
					serviceId, "--attributes", "IPV4_ADDRESS=10.0.0.1,VPC_ID=" + vpcId, "--region", REGION);
		}
		catch (IOException | InterruptedException e) {
			throw new RuntimeException(e);
		}
	...
	public static List<String> getCloudMapSpringBootProperties(LocalStackContainer localStackContainer) {
		...
		properties
				.add("--spring.cloud.aws.cloudmap.endpoint=" + localStackContainer.getEndpointOverride(SSM).toString());
